...
class Sequential(torch.nn.Module): # matrix
...
class ChargeEquilibrium_mod(torch.nn.Module):
    """Charge equilibrium within batches of molecules."""

    def __init__(self):
        super(ChargeEquilibrium_mod, self).__init__()

    def forward(self, g, total_charge=0.0, constraints=None):
        import dgl

        # calculate s^{-1} and e*s^{-1}
        ...
        if constraints is not None and len(constraints) > 0:
            constrained_indices = torch.tensor(list(constraints.keys()), dtype=torch.long, device=g.device)
            fixed_charges = torch.tensor(list(constraints.values()), device=g.device).unsqueeze(-1)
            g.ndata["q_fixed_mask"] = torch.zeros_like(g.ndata["s"], dtype=torch.bool)
            g.ndata["q_fixed"] = torch.zeros_like(g.ndata["s"])
            g.ndata["q_fixed"][constrained_indices] = fixed_charges
            g.ndata["q_fixed_mask"][constrained_indices] = True
        else:
            g.ndata["q_fixed_mask"] = torch.zeros_like(g.ndata["s"], dtype=torch.bool)
            g.ndata["q_fixed"] = torch.zeros_like(g.ndata["s"])

        # Masked s_inv and e_s_inv for free atoms
        free_mask = ~g.ndata["q_fixed_mask"]
        g.ndata["s_inv_masked"] = g.ndata["s_inv"] * free_mask
        g.ndata["e_s_inv_masked"] = g.ndata["e_s_inv"] * free_mask

        # Compute corrected global sums only over free atoms
        sum_s_inv = dgl.sum_nodes(g, "s_inv_masked")
        sum_e_s_inv = dgl.sum_nodes(g, "e_s_inv_masked")

        # Compute remaining charge to be distributed over free atoms
        fixed_charge_sum = dgl.sum_nodes(g, "q_fixed")
        remaining_charge = total_charge - fixed_charge_sum
        g.ndata["sum_q"] = dgl.broadcast_nodes(g, remaining_charge)
        g.ndata["sum_s_inv"] = dgl.broadcast_nodes(g, sum_s_inv)
        g.ndata["sum_e_s_inv"] = dgl.broadcast_nodes(g, sum_e_s_inv)

        # Compute charges only for free atoms
        def compute_q(node):
            s_inv = node.data["s_inv"]
            e = node.data["e"]
            sum_q = node.data["sum_q"]
            sum_s_inv = node.data["sum_s_inv"]
            sum_e_s_inv = node.data["sum_e_s_inv"]
            q_free = -e * s_inv + s_inv * (sum_q + sum_e_s_inv) / sum_s_inv
            # If atom is fixed, use fixed value; else use q_free
            q = torch.where(node.data["q_fixed_mask"], node.data["q_fixed"], q_free)
            return {"q": q}

        g.apply_nodes(compute_q)
